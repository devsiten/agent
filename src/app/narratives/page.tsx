...
export default function NarrativesPage() {
    ...
    const avgScore = narratives.length > 0
        ...
        : 0
    const topNarrative = narratives.length > 0
        ? narratives.reduce((max, n) => n.score > max.score ? n : max, narratives[0])
        : null
    ...
    return (
        <div className="container py-10 px-4 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
                ...
                <Card className="bg-background border-border/30">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium">Top Narrative</CardTitle>
                        <TrendingUp className="h-4 w-4 text-primary" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{loading ? '--' : topNarrative?.name || '--'}</div>
                        <p className="text-xs text-muted-foreground">{loading ? '' : topNarrative ? `Score: ${topNarrative.score}` : ''}</p>
                    </CardContent>
                </Card>
            </div>
